#!/usr/bin/env python3
"""Official benchmark-side evaluation prompts used by HyperDoc reporting."""
...
# Copied from the LongDocURL official evaluation-side answer extraction helper.
# Keep this template unchanged when reproducing the paper evaluation.
LDU_EXTRACTION_PROMPT_TEMPLATE = """Given the question and analysis, you are tasked to extract answers with required formats from the free-form analysis.
...
# Copied from the DocBench official GPT-judge evaluation implementation.
# Keep this prompt unchanged when reproducing the paper evaluation.
DOCBENCH_JUDGE_SYSTEM_PROMPT = (
    "You are a strict QA evaluator. Decide whether prediction is correct against ground truth. "
    "Be tolerant to paraphrase and formatting, but strict on factual/numeric correctness."
)


# Copied from the DocBench official GPT-judge evaluation implementation.
# The text is unchanged; placeholders are exposed as format fields.
DOCBENCH_JUDGE_USER_PROMPT_TEMPLATE = """Question:
{question}
...
{{"verdict":"correct"|"incorrect","score":0|1,"reason":"short reason"}}"""


def format_ldu_extraction_prompt(question: str, analysis: str) -> str:
    return LDU_EXTRACTION_PROMPT_TEMPLATE.format(question=question, analysis=analysis)


def format_docbench_judge_messages(
    question: str,
    ground_truth: str,
    prediction: str,
) -> List[Dict[str, str]]:
    user_prompt = DOCBENCH_JUDGE_USER_PROMPT_TEMPLATE.format(
        question=question,
        ground_truth=ground_truth,
        prediction=prediction,
    )
    return [
        {"role": "system", "content": DOCBENCH_JUDGE_SYSTEM_PROMPT},
